...
function []= rotation_dp_c(distances, percent,a,c,points,percent_rho,percent_delta,theta)
    ...
    T=[cos(theta) -sin(theta);                %Rotation matrix
    sin(theta) cos(theta)];
    ...
    x=-1:0.001:1;
    y=a*x.^2+ c;
    plot(x,y,'Color',[0.945 0.463 0.4]);       %plot the curve of y=a*x^2+c
    xlabel ('X')
    ylabel ('Y')
    z=a*new_data(:,1).^2+c;                    
    ids=find(new_data(:,2)>z);                 %Select all points in the curve y=a*x^2+c and x greater than y(c) as the density peaks
    ...
    for i=1:length(ids)
        NCLUST=NCLUST+1;
        cl(ids(i))=NCLUST;  
        icl(NCLUST)=ids(i);                    %Computing cluster center
    end
